...
export class FantCategorieComponent implements OnInit {
  ...
  moveWithinLevel(nodes: CategoryNode[], index: number, offset: -1 | 1): void {
    const targetIndex = index + offset;
    if (this.reordering || targetIndex < 0 || targetIndex >= nodes.length) {
      return;
    }

    moveItemInArray(nodes, index, targetIndex);
    this.persistOrder();
  }
